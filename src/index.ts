...
import { PotholeServer } from './server.js'; 
...
async function main() { 
    try { 
        ...
        if (cliOptions.stdio) { 
            ...
            const server = new PotholeServer(config.supabaseUrl, config.supabaseKey); 
        ...
        console.error("Fatal error running Pothole server:", error); 
        process.exit(1); 
    } 
} 
