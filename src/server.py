...
- Hazard analytics over Supabase
- Repair plan estimation
- Severity projections with simple weather factors
"""
...
# Suppress logs for clean output (match schematic behavior)
...
PROFILES = {
    "pothole": {
        "base_h": 1.0, "labor": 90, "equip": 65, "traffic": 55, "mat": 240,
...
def prof(t: Optional[str]):
    return PROFILES.get((t or "").lower(), PROFILES["pothole"])

def sev_scale(s: int):
    s = max(0, min(10, int(s or 3)))
    return max(0.6, min(3.0, 0.6 + 0.25 * s))

def build_plan(h: Dict[str, Any]) -> Dict[str, Any]:
    p = prof(h.get("hazard_type"))
    scale = sev_scale(h.get("severity"))
    hours = p["base_h"] * scale
    mobil = 120
    mat = p["mat"] * (0.6 + 0.4 * scale)
    labor = p["labor"] * hours
    equip = p["equip"] * max(0.6, 0.5 * hours)
    traffic = p["traffic"] * max(0.5, 0.6 * hours)
    sub = mat + labor + equip + traffic + mobil
    cont = 0.12 * sub
    ohp = 0.15 * sub
    total = round(sub + cont + ohp, 2)
    ...
    return {
        "assumptions": {
            ...
            "severity": int(h.get("severity") or 3),
            "scale": round(scale, 2),
        },
        "costs": {
            ...
            "traffic_control": round(traffic, 2),
            "mobilization": mobil,
            "contingency_12pct": round(cont, 2),
            "overhead_profit_15pct": round(ohp, 2),
            "total_estimate": total,
        },
        "crew": p["crew"],
        "tools_materials": p["steps"],
        "plan_steps": steps,
        "schedule": {
            "on_site_hours": round(hours + 0.3, 2),
            "lane_closure_hours": round(max(0.75, 0.6 * hours), 2),
...
def prog_per_week(sev: int, t: Optional[str]):
    base = 0.02 * max(0, min(10, int(sev or 3)))
    mult = {"pothole": 1.4, "flooding": 1.25, "debris": 0.85, "damaged_signage": 0.75}.get((t or "").lower(), 1.1)
    return min(0.25, base * mult)

def weather_mult(precip_mm: float = 0.0, ft: int = 0):
    return 1.0 + min(0.5, precip_mm / 200.0) + min(0.4, 0.05 * ft)

# ---------------------------- Tools Block -------------------------------

with server.binding():

    @tool(description="Answer analytics over hazards (areas, severity, types, open vs resolved).")
    def query_hazards(kind: str, location: Optional[str] = None, limit: int = 10) -> Dict[str, Any]:
        """
        - `area_with_most_hazards`: returns top X locations with most hazards. Use `limit` for X. Use `location` to filter by region.
    ...
    @tool(description="Return a type-aware cost estimate and detailed plan for a hazard UUID.")
    ...
    @tool(description="Project severity into the future for a hazard ID or for all hazards in a location.")
    def project_worsening(
        ...
        w = weather_mult(precip_mm, freeze_thaw_cycles)
        ...
        if hazard_id:
            ...
            p = prog_per_week(sev0, h.get("hazard_type"))
        ...
        for h in rows:
            ...
            if len(samples) < 50:
                samples.append({
                    ...
                    "severity_now": sev0,
                    "severity_projected": round(sev_proj, 2),
                })
        ...
        return {
            ...
            "summary": {
                ...
                "avg_severity_now": round(now_sum / n, 2),
                "avg_severity_projected": round(proj_sum / n, 2),
...
async def main() -> None:
    ...
    print("🚀 Starting Hazard MCP Server...")
    print("📡 Transport: streamable-http")
    print("🔧 Available tools: query_hazards, estimate_repair_plan, project_worsening")
    print()
